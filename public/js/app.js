...
var menuItemClassList = menuElement[0].classList;
var heroHeight = document.getElementById('hero').clientHeight;
var aboutHeight = document.getElementById('about').clientHeight;
var portfolioHeight = document.getElementById('portfolio').clientHeight;
var heroSectionEndPoint = heroHeight;
var aboutSectionEndPoint = heroHeight + aboutHeight;
var portfolioSectionEndPoint = heroHeight + aboutHeight + portfolioHeight;
var selectLanguageDropDownBox = document.getElementById('language_drop_down_box');
var selectLanguageBtn = document.getElementById('lang_btn');
var paths = {
...
var actualScrollPosition = 0;
var action = false;
var languageSelectAction = false;
...
document.addEventListener('scroll', function () {
  ...
  if (action) {
    checkScrollPosition(actualScrollPosition);
    action = false;
  }
});
lightDarkSwitch.addEventListener('click', function (e) {
  var action = htmlClassList.toggle('dark');

  if (action) {
    if (e.target.attributes.length > 1) e.target.firstElementChild.attributes.d.value = paths.moon;else e.target.attributes.d.value = paths.moon;
    checkHash();
  } else {
    if (e.target.attributes.length > 1) e.target.firstElementChild.attributes.d.value = paths.sun;else e.target.attributes.d.value = paths.sun;
    checkHash();
  }
});
...
var setColor = function setColor(oldClassArray, newClassArray) {
  header.classList.replace(oldClassArray[0], newClassArray[0]);
  Object.values(menuElement).forEach(function (elem) {
...
var checkHash = function checkHash() {
  var dark_replacedElement = findReplacedElement(headerClassList, 'dark:shadow-');
  var dark_replacedElement2 = findReplacedElement(menuItemClassList, 'hover:dark:text-');
  var light_replacedElement = findReplacedElement(headerClassList, 'shadow-');
  var light_replacedElement2 = findReplacedElement(menuItemClassList, 'text-');

  switch (location.hash) {
    case '':
      if (htmlClassList.length) {
        setColor([dark_replacedElement, dark_replacedElement2], ['dark:shadow-emerald', 'hover:dark:text-emerald-600']);
      } else {
        setColor([light_replacedElement, light_replacedElement2], ['shadow-emerald', 'text-emerald-600']);
      }
    ...
    case '#about':
      if (htmlClassList.length) {
        setColor([dark_replacedElement, dark_replacedElement2], ['dark:shadow-orange', 'hover:dark:text-orange-600']);
      } else {
        setColor([light_replacedElement, light_replacedElement2], ['shadow-orange', 'text-orange-600']);
      }
    ...
    case '#portfolio':
      if (htmlClassList.length) {
        setColor([dark_replacedElement, dark_replacedElement2], ['dark:shadow-primary', 'hover:dark:text-blue-600']);
      } else {
        setColor([light_replacedElement, light_replacedElement2], ['shadow-primary', 'text-blue-600']);
      }
    ...
    case '#contact':
      if (htmlClassList.length) {
        setColor([dark_replacedElement, dark_replacedElement2], ['dark:shadow-golden', 'hover:dark:text-GoldenRod-500']);
      } else {
        setColor([light_replacedElement, light_replacedElement2], ['shadow-golden', 'text-GoldenRod-600']);
      }
...
selectLanguageBtn.addEventListener('mousedown', function () {
  languageSelectAction = true;
});
document.addEventListener('mouseup', function () {
  setTimeout(function () {
    if (selectLanguageDropDownBox.classList.contains('visible') && languageSelectAction) {
      document.addEventListener('click', function (e) {
        var languageParentIds = ['language-item-en', 'language-item-hu'];
        var span_id = e.target.parentElement.id;
        var svg_id = e.target.parentElement.parentElement.id;

        if (selectLanguageDropDownBox.classList.contains('visible') && !languageParentIds.includes(span_id) || !languageParentIds.includes(svg_id)) {
          selectLanguageDropDownBox.classList.replace('visible', 'invisible');
        }
      });
    }
